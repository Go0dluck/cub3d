...
/*   Updated: 2021/01/06 19:26:54 by ksharee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/cub3d.h"

void	pars_color_floor(t_all *all, char **color)
{
...
void	pars_floor(char **str, t_all *all)
{
	...
	all->col_f.kol > 0 ? ft_error("Дубликат цвета пола", all) : 0;
	if (ft_split_size(str) == 2)
	{
		if (ft_split_size(color = ft_split(str[1], ',')) == 3)
		{
			...
			{
				free_split(color);
				ft_error("Не верный формат RGB цвета пола", all);
			}
			pars_color_floor(all, color);
			free_split(color);
		}
		else
			ft_error("Не верный формат RGB цвета пола", all);
		...
		ft_error("Не верные параметры цвета пола", all);
}
